 ...
 * @return {[type]} [返回对象]
 */
Object.prototype.shallowClone = function(origin) {
    var target = {};
...
Object.prototype.pesudoToArray = function() {
    ...
    //方法 6 ： ES6 -- 扩展运算符 
    var args6 = [...that];
    return (args4 || args1 || args2 || args5 || args6);
};
...
Element.prototype.getParent = function() {
    var e = this,
        parent = e && (e.parentElement || e.parentNode);
    return parent;
}
/**
 * [getChildren 返回所有元素节点]
...
var EventUtil = {
    ...
    /**
     * [storageEvent storage事件]
     * @param  {[type]} handle [storage处理函数]
     * @return {[type]}        [description]
     */
    storageEvent: function(handle) {
...
var DOMUtil = {
    /**
     * [getDom 查找DOM方法1]
     * @param  {[type]} elName [DOM的 id || className || tagName || name]
     * @param  {Number} i      [查找所有的结果集中的某一个]
     * @return {[type]}        [description]
     */
    getDom: function(elName, i = 0) {
        var byId = document.getElementById(elName),
            byClass = document.getElementsByClassName(elName)[i], //HTMLCollection
            byTag = document.getElementsByTagName(elName)[i], //HTMLCollection 
            byName = document.getElementsByName(elName)[i]; //NodeList 
        return byId || byClass || byTag || byName;
    },
    /**
     * [queryDom 查找DOM方法2]
     * @param  {[type]} elSelector [指定一种选择器]
     * @param  {Number} i      [查找所有的结果集中的某一个]
     * @return {[type]}        [description]
     */
    queryDom: function(elSelector, i = 0) {
        var selector = document.querySelector(elName), //返回第一个满足
            selectorAll = document.querySelectorAll(elName)[i]; //NodeList
...
/**
 * [debounce 防抖函数]
 * @param  {[type]} fn [触发事件函数]
 * @param  {[type]} delay  [延迟执行时间]
 * @return {[type]}        [description]
 */
 ...
 * [throttle 节流函数]
 * @param  {[type]} fn    [触发事件函数]
 * @param  {[type]} delay [触发时间间隔]
 * @return {[type]}       [description]
 */
function throttle(fn, delay) {
    let timer,
        start = new Date(); //上次触发时间
    return function() {
        let self = this,
            args = arguments,
            delay = delay || 150,
            now = new Date() - 0; //本次触发时间
        clearTimeout(timer); //清除上次节流函数，重新计算
        let loop = delay - (now - start);
        if (loop <= 0) {
            fn.apply(self, args); //第一次触发立即执行
            start = now;
        } else {
            timer = setTimeout(function() {
                fn.apply(self, args); //触发之后，进入计时，期间不允许执行
            }, delay);
        }
    }
}
 ...
 */
/**
 * [getTop 获取当前元素在页面中的位置，循环获取直到document顶部]
 * @param  {[type]} e [目标元素]
 * @return {[type]} T  [当前元素距离页面顶部偏移量]
 */
function getTop(e) {
    //自身在父元素中偏移量
    var T = e.offsetTop;
    //循环获取祖先偏移量，并添加到当前元素偏移量
...
function lazyLoadImage() {
    ...
    for (let i = 0, len = imgs.length; i < len; i++) {
        ...
        if (H + S > getTop(imgs[i])) {
            //将在可是区域内的图片自定义属性data-src 值添加到 src属性中
